if (contriblyGalleryWidgetLoaded == undefined) {
var contriblyGalleryWidgetLoaded = true;
...
requirejs.config({
    baseUrl: 'https://s3-eu-west-1.amazonaws.com/contribly-widgets/gallery/lib',
    paths: {
        jquery: 'jquery-3.1.1',
    ...
    map: {
      'fancybox': { 'jquery': 'jquery' },
      'jquery-dateFormat': { 'jquery': 'jquery' }
    }
});

requirejs(['jquery-private', 'underscore'], function(contriblyGalleryjQuery, contriblyUnderscore) {
    ...
    function contriblyInitGallery(span) {
      ...
      contriblyApi = (overrideContriblyApi) ? overrideContriblyApi : "https://contriblyapi.global.ssl.fastly.net/1",
    ...
    var contriblyGalleryPageSize = 30;
    ...
    var infiniteScroll = {
      ...
      initialize: function() {
        ...
        this.loadData(assignmentUrl).done(function() {
            contriblyGalleryjQuery('.fancybox').fancybox({    // TODO Scope to this widget only
                beforeLoad: function() {
                    var currentPosition = this.index + 1;
                    var numberLoaded = contriblyGalleryjQuery('.list').children().length;
                    var shouldLoadMore = totalCount > 0 && (currentPosition == (numberLoaded));
                    if(shouldLoadMore) {
                      var eventuallyLoaded = infiniteScroll.loadData(assignmentUrl);
                      contriblyGalleryjQuery.when(eventuallyLoaded).then(function(contributions) {
                            var fancybox = contriblyGalleryjQuery.fancybox;
                            contriblyGalleryjQuery(contributions).each(function(index, contribution) {
                                ...
                                var element = galleryItemForContribution.find(".fancybox");
                                var fancyBoxItem = {
                                    type: "image",
                                ...
                                fancybox.group.push(fancyBoxItem);
                            });
                      });
                    }
      ...
      loadData: function(assignmentUrl) {
        ...
        var eventualContributions = contriblyGalleryjQuery.ajax({
          type: 'GET',
          url: ajaxUrl
        }).promise();

        eventualContributions.done(function(contributions) {
    ...
    contriblyGalleryjQuery('.contribly-gallery').each(function(i, v) {
        ...
        var cssToLoad = (requestedCss != undefined) ? requestedCss : "https://s3-eu-west-1.amazonaws.com/contribly-widgets/gallery/gallery2017030701.css";
        contriblyGalleryjQuery.ajax({
            url: cssToLoad,
